...
class NovoGrad(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=0.01, betas=(0.95, 0.98), eps=1e-8, weight_decay=0, grad_averaging=False):
        if lr < 0.0:
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if len(state) == 0:
                    state['step'] = 0
                    state['moments'] = grad.div(g_2.sqrt() + group['eps']) + group['weight_decay'] * p.data
                    state['grads_ema'] = g_2
